...
class SampleSerializer extends Serializer[Sample]{
  ...
  override def write(kryo: Kryo, output: Output, t: Sample): Unit = output.writeString(t.toJson.compactPrint)
  override def read(kryo: Kryo, input: Input, aClass: Class[Sample]): Sample = input.readString().parseJson.convertTo[Sample]
}
